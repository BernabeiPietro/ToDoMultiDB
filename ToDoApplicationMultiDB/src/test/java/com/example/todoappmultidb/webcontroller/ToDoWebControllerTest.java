...
public class ToDoWebControllerTest {
	...
	public void test_UserToDoView_ShowUserToDo() throws Exception {
		...
		when(userService.getToDoOfUser(1l)).thenReturn(todo);
	...
	public void test_UserToDoView_EmptyUserToDo() throws Exception {
		when(userService.getToDoOfUser(1l)).thenThrow(new NotFoundException("Not found User ToDo  with id 1"));
...
//edit
	...
	public void test_EditToDo_IdFound() throws Exception {
		...
		when(todoService.findByIdDTO(1l)).thenReturn(value);
	...
	public void test_EditToDo_IdNotFound() throws Exception {
		when(todoService.findByIdDTO(1l)).thenThrow(new NotFoundException("Not found todo with id 1"));
...
//edit_db
	...
	public void test_EditToDo_db_2() throws Exception {
		InOrder inOrder = Mockito.inOrder(userService, todoService);
		when(todoService.findByIdDTO(1l)).thenReturn(notImportant);
...
//save new_db
	...
	public void test_saveTodo_db_default() throws Exception {
		...
		mvc.perform(post("/todo/save").param(ID, "-1").param("idOfUser", "1").param("date", "2005-01-12 00:00:00"))
				.andExpect(view().name("redirect:/todo/ofuser/1"));
		inOrder.verify(userService).setDatabase(1);
		inOrder.verify(todoService).save(new ToDoDTO(null, 1l, new HashMap<>(), LocalDateTime.of(2005, 1, 12, 0, 0)));
	}
...
//update_db
	...
	public void test_UpdateToDo_db_default() throws Exception {
		...
		mvc.perform(post("/todo/save").param(ID, "1").param("idOfUser", "1").param("date", "2005-01-12 00:00:00")
				...
				.andExpect(view().name("redirect:/todo/ofuser/1"));
		inOrder.verify(userService).setDatabase(1);
		inOrder.verify(todoService).updateById(1l,
...
//add task
	...
	public void test_PostAddNewTask_fullMap_nullKey_fullValue() throws Exception {
		...
		mvc.perform(post("/todo/addaction").param(ID, "1").param("idOfUser", "1")
				.param("actions", "{first=false,second=true}").param("date", "2005-01-12 00:00:00")
				.param("value", "true")).andExpect(view().name("editToDo")).andExpect(model().attribute(MESSAGE, ""))
				.andExpect(
						model().attribute("todo", new ToDoDTO(1l, 1l, actions, LocalDateTime.of(2005, 1, 12, 0, 0))));
	}
	...
	public void test_PostAddNewTask_fullMap_fullKey_emptyValue() throws Exception {
		...
		mvc.perform(post("/todo/addaction").param(ID, "1").param("idOfUser", "1")
				.param("actions", "{first=false,second=true}").param("date", "2005-01-12 00:00:00")
				.param("key", "prova")).andExpect(view().name("editToDo")).andExpect(model().attribute(MESSAGE, ""))
				.andExpect(
						model().attribute("todo", new ToDoDTO(1l, 1l, actions, LocalDateTime.of(2005, 1, 12, 0, 0))));
	}
	...
	public void test_PostAddNewTask_fullMap_emptyKey_fullValue() throws Exception {
		...
		actions.put("second", true);
		mvc.perform(post("/todo/addaction").param(ID, "1").param("idOfUser", "1")
				.param("actions", "{first=false,second=true}").param("date", "2005-01-12 00:00:00").param("key", "")
				.param("value", "true")).andExpect(view().name("editToDo")).andExpect(model().attribute(MESSAGE, ""))
				.andExpect(
						model().attribute("todo", new ToDoDTO(1l, 1l, actions, LocalDateTime.of(2005, 1, 12, 0, 0))));
	}
...
//error function
	...
	public void test_PostToDoWithId_notFoundExceptionByUpdate() throws Exception {
		doThrow(new NotFoundException("Not found any todo with id 1")).when(todoService).updateById(1l,
				new ToDoDTO(1l, 1l, new HashMap<>(), LocalDateTime.of(2005, 1, 12, 0, 0)));
		mvc.perform(post("/todo/save").param(ID, "1").param("idOfUser", "1").param("date", "2005-01-12 00:00:00"))
	...
	public void test_PostToDoWithoutId_notFoundExceptionBySave() throws Exception {
		doThrow(new NotFoundException("Not found any user with id 1")).when(todoService)
				.save(new ToDoDTO(null, 1l, new HashMap<>(), LocalDateTime.of(2005, 1, 12, 0, 0)));
		mvc.perform(post("/todo/save").param(ID, "-1").param("idOfUser", "1").param("date", "2005-01-12 00:00:00"))
