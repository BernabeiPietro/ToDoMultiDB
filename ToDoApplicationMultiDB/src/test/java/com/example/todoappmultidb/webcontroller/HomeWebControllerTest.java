...
public class HomeWebControllerTest {
	...
	public void test_HomeView_ShowsUsers() throws Exception {
		List<UserDTO> users = asList(new UserDTO(1L, new ArrayList<ToDoDTO>(), "test", "test"));
	...
	public void test_EditUser_WhenUserIsFound() throws Exception {
		UserDTO user = new UserDTO(1L, new ArrayList<ToDoDTO>(), "test", "test");
		when(userService.getUserById(1L)).thenReturn(user);
		mvc.perform(get("/edit/1")).andExpect(view().name("edit")).andExpect(model().attribute("user", user))
				.andExpect(model().attribute(MESSAGE, ""));
	}
	...
	public void test_EditUser_WhenUserIsNotFound() throws Exception {
		...
		mvc.perform(get("/edit/1")).andExpect(view().name("edit")).andExpect(model().attribute("user", nullValue()))
				.andExpect(model().attribute(MESSAGE, "No user found with id: 1"));
	}

	@Test
	public void test_EditNewUser() throws Exception {
		mvc.perform(get("/new")).andExpect(view().name("edit")).andExpect(model().attribute("user", new UserDTO()))
				.andExpect(model().attribute(MESSAGE, ""));
	...
	public void test_PostUserWithoutId_ShouldInsertNewUser() throws Exception {
		mvc.perform(post("/save").param("name", "test name").param("email", "test email"))
				.andExpect(view().name("redirect:/"));
		verify(userService).insertNewUser(new UserDTO(null, new ArrayList<>(), "test name", "test email"));
	}

	@Test
	public void test_PostUserWithId_ShouldUpdateExistingUser() throws Exception {
		mvc.perform(post("/save").param("id", "1").param("name", "test name").param("email", "test email"))
				.andExpect(view().name("redirect:/"));
		verify(userService).updateUserById(1L, new UserDTO(1L, null, "test name", "test email"));
	}
}
