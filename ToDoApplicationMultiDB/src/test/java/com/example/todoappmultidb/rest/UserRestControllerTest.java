...
/////////////////////	
	...
	public void testPostInsertNewUser_db1() throws Exception {
		...
		when(userService.insertNewUser(userToSave)).thenReturn(new UserDTO(1l, "nome1", "email1"));
	...
	public void testPostInsertNewUserNull_db1() throws Exception {
		...
		when(userService.insertNewUser(userToSave)).thenThrow(new IllegalArgumentException("User with null property"));
		this.mvc.perform(post("/api/users/1/new").content(objMapper.writeValueAsString(userToSave))
				.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON))
	...
	public void testPutUpdateUser_db1() throws Exception {
		...
		when(userService.updateUserById(1, userToUpdate)).thenReturn(new UserDTO(1l, "nome1", "email1"));
