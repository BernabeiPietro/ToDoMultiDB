...
public class ToDoRepositoryTest {
	...
	public void test_save() {
		...
		assertThat(result).hasNoNullFieldsOrProperties().hasFieldOrPropertyWithValue("toDo", todo1.getToDo())
				.hasFieldOrPropertyWithValue("idOfUser", todo1.getIdOfUser())
				.hasFieldOrPropertyWithValue("date", LocalDateTime.of(2012, Month.DECEMBER, 12, 0, 0));
	}
	...
	public void test_retrieveUserListOfTodoWithALimitedUserId() {
		...
		ToDo result1 = entityManager.persistAndFlush(todo1);
		ToDo result2 = entityManager.persistAndFlush(todo2);
		assertThat(toDoRepository.findToDoByUserId(new User(user.getId(), null, null, null)))
				.isEqualTo(asList(result1, result2));
	}
